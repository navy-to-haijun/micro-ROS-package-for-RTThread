...
static void microros_sub_int32_thread_entry(void *parameter)
{
    while(1)
    {
        ...
        rt_kprintf("进入循环：\t");
        rt_int32_t a;
        a = rclc_executor_spin_some(&executor, RCL_MS_TO_NS(100));
...
static void microros_sub_int32(int argc, char* argv[])
{
    ...
    a = rclc_executor_add_subscription(&executor, &subscriber, &msg, &subscription_callback, ON_NEW_DATA);
    rt_kprintf("[micro_ros] executor created\t a = %d\n", a);
