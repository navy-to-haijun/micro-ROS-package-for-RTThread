...
static void microros_pub_int32(int argc, char* argv[])
{
    ...
    // create publisher
    flag = rclc_publisher_init_default(
      &publisher,
      &node,
    ...
    if(flag != RCL_RET_OK)
    {
        rt_kprintf("[micro_ros] failed to create timer\n");
        return;
    }
    rt_kprintf("[micro_ros] timer created\n");

    // create executor
    flag = rclc_executor_init(&executor, &support.context, 1, &allocator);
    rclc_executor_add_timer(&executor, &timer);
    if(flag != RCL_RET_OK)
    {
        rt_kprintf("[micro_ros] failed to create executor\n");
        return;
    }
